...
async fn handle_command(state: &mut State, command: Command) -> Result<Response> {
    ...
    match command {
        ...
        Command::ClipboardCopy { selector, timeout } => {
            ...
            // JS that copies text or image content to the clipboard.
        ...
        Command::ClipboardPaste => {
            ensure_clipboard_permissions(state).await?;
            // Read clipboard to determine content type, then dispatch a paste event
            // with the correct data. This works regardless of platform key bindings.
            let js = r#"async () => {
